...
export class BlackSquareBonanzaB36Handler implements PlaneEventHandler {
  ...
  constructor(simConnectConnection: SimConnectConnection, handlerOptions?: Record<string, any>) {
    ...
    this.availableEvents = availableEvents
      .find((a) => a.aircraft === "Blacksquare Bonanza B36")!
  ...
  start(): void {
    console.log("will start websocket connection for Bonanza B36 event handler");
    this.ws = new WebSocket(this.wsAddress, "fsuipc");
    this.ws.onopen = async () => {
      console.log(
        "WebSocket connection established for Bonanza B36 event handler",
      );
    };
