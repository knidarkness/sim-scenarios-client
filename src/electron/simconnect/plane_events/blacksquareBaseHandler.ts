...
export abstract class BlackSquareBaseHandler implements PlaneEventHandler {
  ...
  protected readonly aircraftName: string;
  ...
  start(): void {
    console.log(`will start websocket connection for ${this.aircraftName} event handler`);
    this.ws = new WebSocket(this.wsAddress, "fsuipc");
    this.ws.onopen = async () => {
      console.log(
        `WebSocket connection established for ${this.aircraftName} event handler`,
      );
    };
